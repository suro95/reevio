...
function applyFrameBlur(value) {
    ...
    //var gradientPixels = getRadialGradientMap(canvas.width, canvas.height, canvas.width * 0.5, canvas.height * 0.5, outerRadius / 2, outerRadius);
...
function applyFrame(frame) {
    ...
    var imageData = context.getImageData(0, 0, canvas.width, canvas.height),
        data = imageData.data;
    var mask = new Image();
    mask.src = "/vendor/Lollipop/assets/img/masks/" + frame + ".jpg";
    var maskData;
...
function applySpecial(special) {
    ...
    var imageData = context.getImageData(0, 0, canvas.width, canvas.height),
        data = imageData.data;
    var mask = new Image();
    mask.src = "/vendor/Lollipop/assets/img/Special/" + special + ".jpg";
    var maskData;
    mask.onload = function () {
        var tempCanvas = document.createElement('canvas');
        var width = canvas.width;
